...
def solution(words):
    ...
    #         leng = i + 1
    #         if word[:leng] not in [words[j][:leng] for j in range(len(words)) if words[j] != word]:
    #             break
    #     answer += leng
    answer =0 
    trie = Trie()
    ...
    for word in words:
        ...
        for i in range(len(word)):
            leng = i + 1
            possible = trie.starts_with(word[:leng])
            # print(possible)
            if len(possible) == 1 and possible[0] == word:
                break
        answer += leng
    return answer
